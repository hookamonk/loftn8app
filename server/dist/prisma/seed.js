...
async function upsertCategory(venueId, name, sort, section) {
...
async function upsertItemInCategory(categoryId, data) {
    ...
    if (existing) {
        ...
        const nextImageUrl = data.imageUrl !== undefined ? data.imageUrl : (existing.imageUrl ?? null);
...
async function main() {
    ...
    for (const [name, sort] of DISHES) {
        const c = await upsertCategory(venue.id, name, sort, client_1.MenuSection.DISHES);
        cats.set(name, c);
    }
    for (const [name, sort] of DRINKS) {
        const c = await upsertCategory(venue.id, name, sort, client_1.MenuSection.DRINKS);
        cats.set(name, c);
    }
    for (const [name, sort] of HOOKAH) {
        const c = await upsertCategory(venue.id, name, sort, client_1.MenuSection.HOOKAH);
        cats.set(name, c);
    }
    ...
    for (const legacyName of ["SPIRITS", "WINE", "HOT DRINKS"]) {
    ...
    const itemsByCategory = {
        ...
        "APPETIZERS / SNACKS": [
            ...
            {
                name: "BEEF CARPACCIO",
                description: "Argentinian aged filet, parmesan, arugula, crispy baguette, olive oil (1,3,7,11) / 110g",
                priceCzk: 445,
                sort: 3,
            ...
            },
            {
                ...
                description: "Chopped Argentinian aged filet, capers, balsamic, olive oil, marinated cucumber, red onion, crispy garlic toast, quail eggs (1,3,6) / 110g",
                priceCzk: 435,
                sort: 4,
            ...
            },
            {
                ...
                description: "Pancakes, salmon, Philadelphia cheese, mayo, chives (1,3,4,7,10) / 4 pcs",
                priceCzk: 195,
                sort: 10,
        ...
        "SALADS / SOUPS": [
            {
                ...
                description: "110g Argentina aged filet (medium), baked vegetables, champions, olive oil, balsamico, microgreens (6) / 350g",
                priceCzk: 445,
                sort: 1,
            ...
            {
                name: "GREEK SALAD",
                description: "Feta cheese, kumato tomatoes, cucumber, red onions, pepper, dill, cilantro, Spanish olives, olive oil (7) / 410g",
                priceCzk: 285,
                sort: 4,
            ...
            },
            {
                ...
                description: "cold soup based on kefir, vegetables and beef, served with garlic bread, and lemon (1,3,7) / 0,35l",
                priceCzk: 145,
                sort: 17,
        ...
        "FISH / MEAT": [
            ...
            {
                name: "BUFFALO WINGS",
                description: "Best seller! Juicy, moderately sweet and spicy wings baked in the oven, with Blue Cheese sauce and roughly chopped vegetables. Recommend French fries and sauce for them (7) / 500g",
                priceCzk: 295,
                sort: 5,
        ...
        "BURGERS / SANDWICHES": [
            {
                ...
                description: "smoked chicken jam, arugula, aioli (1,3,7,10,11) / 150g Recomended: french fries and Coca-Cola 0,3l ",
                priceCzk: 295,
                sort: 1,
            ...
            },
            {
                ...
                description: "cheddar, tomato, onion, Lollo salad, mayo, pickled cucumber (1,3,7,10,11) / 150g Recomended: french fries and Coca-Cola 0,3l",
                priceCzk: 285,
                sort: 2,
            ...
            },
            {
                ...
                description: "provolone, parmesan, jalapeño, habañero, smoked chicken jam, slaw (1,3,7,10,11) / 150g Rewcomended: french fries and Coca-Cola 0,3l",
                priceCzk: 325,
                sort: 3,
    ...
    for (const [catName, items] of Object.entries(itemsByCategory)) {
        ...
        if (!cat)
            throw new Error(`Seed error: category not found "${catName}"`);
    ...
    for (const s of staffSeed) {
        const passwordHash = await bcryptjs_1.default.hash(s.password, 10);
        await prisma.staffUser.upsert({
            where: { username: s.username },
            update: { role: s.role, venueId: venue.id, passwordHash, isActive: true },
            create: { role: s.role, venueId: venue.id, username: s.username, passwordHash, isActive: true },
        });
    }
