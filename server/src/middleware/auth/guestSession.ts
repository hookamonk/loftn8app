...
export async function guestSessionAuth(
	req: Request,
	_res: Response,
	next: NextFunction,
) {
	...
	if (!gsid)
		return next(
			new HttpError(401, "NO_GUEST_SESSION", "Guest session is required"),
		);

	let guestPayload: GuestPayload;
	try {
		guestPayload = jwt.verify(
			gsid,
	...
	if (!session || session.endedAt) {
		return next(
			new HttpError(401, "SESSION_NOT_FOUND", "Session not found or ended"),
		);
	}
	...
	if (requestedVenueSlug) {
		...
		const sessionVenueSlug = normalizeVenueSlug(
			session.table.venue?.slug ?? null,
		);
		if (sessionVenueSlug !== requestedVenue) {
			return next(
				new HttpError(
					...
					"Guest session belongs to another branch",
				),
			);
		}
	}
