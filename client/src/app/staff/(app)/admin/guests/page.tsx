...
export default function StaffAdminGuestsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    setErr(null);
    const r = await getAdminUsers(range, venue);
    if (!r.ok) {
      setErr(r.error || "Не удалось загрузить гостей");
      setLoading(false);
      return;
    }
  ...
  if (!isAdmin) {
    return (
      <div className="rounded-[24px] border border-white/10 bg-white/[0.04] p-6 text-white">
        <div className="text-lg font-semibold">Гости</div>
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-[24px] border border-white/10 bg-white/[0.05] p-5 space-y-4">
        ...
        <div>
          <div className="mb-2 text-xs uppercase tracking-[0.14em] text-white/45">Регистрация за</div>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="space-y-2.5 lg:hidden">
            {filtered.map((u) => (
              <div key={u.id} className="rounded-[20px] border border-white/10 bg-white/[0.04] p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="shrink-0 text-right">
                    <div className="text-base font-semibold text-emerald-300">{u.bonusCzk} Kč</div>
                    <div className="text-[10px] uppercase tracking-[0.14em] text-white/35">бонусы</div>
                  </div>
                </div>
              </div>
            ))}
                ...
                {filtered.map((u) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5 text-right">
                      <span className="font-semibold text-emerald-300">{u.bonusCzk} Kč</span>
                      {u.pendingBonusCzk > 0 ? (
                        <span className="ml-2 text-[11px] text-white/35">+{u.pendingBonusCzk} ждёт</span>
                      ) : null}
                    </td>
                  </tr>
                ))}
